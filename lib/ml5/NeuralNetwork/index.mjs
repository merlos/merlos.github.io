...
import NeuralNetworkData from './NeuralNetworkData.mjs';
import NeuralNetworkVis from './NeuralNetworkVis.mjs';
import callCallback from '../utils/callcallback.mjs';

import nnUtils from './NeuralNetworkUtils.mjs';
import { imgToPixelArray, isInstanceOfSupportedElement } from '../utils/imageUtilities.mjs';
...
class DiyNeuralNetwork {
  ...
  async init(callback) {
    ...
    if (this.options.dataUrl !== null) {
      this.ready = await this.loadDataFromUrl(this.options, callback);
  ...
  loadDataFromUrl(options, callback) {
    ...
    return this.loadDataInternal(options);
  }
  ...
  createMetaData = function(dataRaw) {
  ...
  async train(optionsOrCallback, optionsOrWhileTraining, callback) {
    ...
    await this.trainInternal(options, whileTrainingCb, finishedTrainingCb);
  }
   ...
   * @param {*} _cb
   */
  async trainInternal(_options, whileTrainingCb, finishedTrainingCb) {
    const options = {
      epochs: 10,
      batchSize: 32,
      validationSplit: 0.1,
    ...
    await this.neuralNetwork.train(options, finishedTrainingCb);
  }
